...
local V=game:GetService( "ProximityPromptService" )pcall(function(...) V.PromptButtonHoldBegan :Connect(function(e,...) pcall(function(...)
            if typeof(fireproximityprompt)== "function" then
                fireproximityprompt(e)endend)end)end)
local H=function(...)end
local t=function(...)end
local s=nil pcall(function(...) s=require((j:WaitForChild( "Client" , 5 )):WaitForChild( "EggState" , 5 ))end)
if not s then
    pcall(function(...) s=require(j.Client.EggState )end)end
local p=nil pcall(function(...) p=require(((j:WaitForChild( "Shared" , 5 )):WaitForChild( "Util" , 5 )):WaitForChild( "AssetItems" , 5 ))end)
if not p then
    pcall(function(...) p=require(j.Shared.Util .AssetItems )end)end
local B=nil pcall(function(...) B=require((j:WaitForChild( "Shared" , 5 )):WaitForChild( "Remotes" , 5 ))end)
if not B then
    pcall(function(...) B=require(j.Shared.Remotes )end)end
...
local Z= "DiceHubFlightSpeed.txt"
local z= "DiceHubEggSelectConfig.json"
...
local h
local function O(...)
    local e= 1000 pcall(function(...)
        local r= false
        if isfile then
            r=isfile(Z)
        elseif readfile then
            local e,y=pcall(readfile,Z)r=e and(y~=nil)end
        if r and readfile then
            local r=readfile(Z)
            local u=tonumber(r)
            if u and(u>= 100 and u<= 1000 )then
                e=math.floor (u)endendend)
    return eend
local function Y(e,...) pcall(function(...)
        if writefile then
            local y=math.clamp (math.floor (tonumber(e)or 600 ), 100 , 1000 )writefile(Z,tostring(y))endend)end
local function T(...)
...
local function x(...) pcall(function(...)
        if writefile and(a and h)then
            ...
            local y=a:JSONEncode(r)writefile(z,y)endend)end
...
local W4=nil pcall(function(...)
    ...
    (e:GetPropertyChangedSignal( "ClockTime" )):Connect(function(...) X4={}G4= 0end)end
)pcall(function(...)
    local function e(e,...)
        if e:IsA( "RemoteEvent" )then
            ...
            if string.find (y, "reset" )or string.find (y, "night" )or string.find (y, "spawn" )or string.find (y, "countdown" )then
                pcall(function(...) e.OnClientEvent :Connect(function(...) X4={}G4= 0end)end)endendend
    for y,u in ipairs(j:GetDescendants())do
        e(u)end
    j.DescendantAdded :Connect(e)end
)m=function(e,...)
...
u4=function(e,...)
    ...
    if y then
        pcall(function(...) y:UnequipTools()end)end
    if r and u then
        for e,r in ipairs(r:GetChildren())do
            if r:IsA( "Tool" )then
                pcall(function(...) r.Parent =uend)endendendend
w4=function(e,...)
                ...
                for r,y in pairs(r)do
                    if type(y)== "table" then
                        if not y.Uid and type(r)== "string" then
                            y.Uid =rend
                        table.insert (e,y)endendend
            if#e> 0 then
                F4=e G4=os.clock ()endend
        m4= falseend)end
task.spawn (function(...)
    while true do
        task.wait ( 1.5 )pcall(ek)endend
)function h4(e,...)
    local y=os.clock ()
    ...
    if u then
        for e,r in ipairs(u)do
            if r.Uid then
                k[r.Uid ]= true table.insert (j,r)endendend
    if w then
                        ...
                        for u,w in ipairs(j)do
                            if w.Uid ==y then
                                ...
                                if not w.AreaId or w.AreaId == "" or w.AreaId == "Unknown" then
                                    w.AreaId =r:GetAttribute( "AreaId" )or r:GetAttribute( "Area" )end
                                breakendendendendendendend
    return jend
k4=function(e,...)
    if not e then
        return false , "NoUid"end
    local y=h4( false )
    if y and#y> 0 then
...
o4=function(...)
    ...
    if not r then
        local e,y=r4()r=yend
    if not r then
        return h.glideSpeed or 350end
    if s and s.ReadFieldEggs then
        local e,u=pcall(s.ReadFieldEggs )
        if e and(u and u.Records )then
            for e,u in ipairs(u.Records )do
                if u.Uid ==r and u.AreaId then
                    return I[u.AreaId ]or h.glideSpeed or 350endendendend
    return h.glideSpeed or 350end
V4=function(e,y,...) y=y or 8
    local u=Instance.new ( "Part" )u.Name = "SafetyFloorPadAntiVoid" u.Size =Vector3.new ( 28 , 1.5 , 28 )u.Position =e-Vector3.new ( 0 , 3.2 , 0 )u.Anchored = true u.Transparency = 1 u.CanCollide = true u.Parent =r task.delay (y,function(...) pcall(function(...) u:Destroy()end)end)
    return uend
H4=function(e,...)
    if P and e then
        pcall(function(...)
            ...
            if P:IsA( "RemoteFunction" )then
                ...
                [ "GuardCFrame" ]=u})endend)endend
if typeof(hookmetamethod)== "function" and not G.DesyncAntiRagdollHooked then
    G.DesyncAntiRagdollHooked = true
    local e e=hookmetamethod(game, "newindex" ,safeNewCClosure(function(r,y,u,...)
...
S4=function(e,...) e=e or o.Character
    ...
    for e,r in ipairs(e:GetDescendants())do
        if r:IsA( "BallSocketConstraint" )or r:IsA( "HingeConstraint" )or r:IsA( "NoCollisionConstraint" )then
            pcall(function(...) r:Destroy()end)endend
    for e,r in ipairs(e:GetDescendants())do
        if r:IsA( "Motor6D" )and(r.Part0 and r.Part1 )then
            r.Enabled = true
            local e= "RigidJointWeld" ..r.Name
            local y=r.Part1 :FindFirstChild(e)
            if not y then
                local y=Instance.new ( "WeldConstraint" )y.Name =e y.Part0 =r.Part0 y.Part1 =r.Part1 y.Parent =r.Part1endendendend
Z4=function(e,...)
        ...
        pcall(function(...)
            for r,y in ipairs(r:GetPlayingAnimationTracks())do
                ...
                if string.find (w, "10921259953" )or string.find (string.lower (y.Name ), "treadmill" )or string.find (string.lower (y.Name ), "run" )then
                    y:Stop( 0 )endend
            r.PlatformStand = false r.Sit = false r:SetStateEnabled(Enum.HumanoidStateType.Running , true )r:SetStateEnabled(Enum.HumanoidStateType.Jumping , true )r:ChangeState(Enum.HumanoidStateType.Running )end)end
    local u=o:FindFirstChild( "PlayerGui" )
    if u then
        local e=u:FindFirstChild( "SpeedGainAnimation" )
        if e then
            pcall(function(...) e:Destroy()end)endend
    if y then
        y.AssemblyLinearVelocity =Vector3.zero y.AssemblyAngularVelocity =Vector3.zeroend
    Z4(e)end
local rk= 0
...
M4=function(...)
    ...
    E4()
    if U then
        pcall(function(...) U:InvokeServer()end)end
    local e=o.Character
    local r=e and e:FindFirstChildOfClass( "Humanoid" )
    local y=e and e:FindFirstChild( "HumanoidRootPart" )
    if r then
    ...
    if y then
        y.AssemblyLinearVelocity =Vector3.zero y.AssemblyAngularVelocity =Vector3.zeroend
    Z4(e)task.wait ( 0.15 )yk= falseend
            ...
            for e,u in ipairs(e:GetChildren())do
                ...
                if j and j:IsA( "BasePart" )then
                    if w and(h and h.autoTreadmill )then
                        ...
                        j.CanTouch = false j.CanCollide = falseendend
                    ...
                    for e,r in ipairs(k:GetDescendants())do
                        if r:IsA( "BasePart" )then
                            if w and(h and h.autoTreadmill )then
                                ...
                                r.CanTouch = false r.CanCollide = falseendendendendendendend)end
n4()r.DescendantAdded :Connect(function(e,...) pcall(function(...)
        ...
        if r or y then
            ...
            if w and(h and h.autoTreadmill )then
                ...
                if r then
                    ...
                    for e,r in ipairs(e:GetDescendants())do
                        if r:IsA( "BasePart" )then
                            r.CanTouch = false r.CanCollide = falseendendendendendend)end
    ...
    local r=e and e:FindFirstChild( "HumanoidRootPart" )
    if r then
        pcall(function(...) r.Anchored = false r.AssemblyLinearVelocity =Vector3.zero r.AssemblyAngularVelocity =Vector3.zeroend)end
    pcall(function(...)
        if C4 then
            C4()endend
    )pcall(function(...)
        ...
        for e,r in ipairs(e:GetDescendants())do
            if r:IsA( "ProximityPrompt" )then
                pcall(function(...) r.RequiresLineOfSight = false r.HoldDuration = 0
                    if typeof(fireproximityprompt)== "function" then
                        fireproximityprompt(r, 0 )fireproximityprompt(r)endend)endendend
    local u=r:FindFirstChild( "AreaEggSlotsClient" )
    if u and y then
            ...
            if u and((u.Position -y)).Magnitude <= 18 then
                for e,r in ipairs(r:GetDescendants())do
                    if r:IsA( "ProximityPrompt" )then
                        pcall(function(...) r.RequiresLineOfSight = false r.HoldDuration = 0
                            if typeof(fireproximityprompt)== "function" then
                                fireproximityprompt(r, 0 )fireproximityprompt(r)endend)endendendendendend
b4=function(e,...) h.godmode =e
    ...
    for r,y in ipairs(r:GetDescendants())do
        if y:IsA( "BasePart" )then
            if e then
                y.CanTouch = false y.CanCollide = falseendendend
    Z4(r)end
...
A4=function(...)
    ...
    pcall(function(...) y.BreakJointsOnDeath = false
        ...
        if not j then
            j=Instance.new ( "Animator" )j.Parent =wend
        r.CurrentCamera.CameraSubject =w
        local k=e:FindFirstChild( "Animate" )
        if k and k:IsA( "LocalScript" )then
            k.Disabled = true task.defer (function(...) task.wait ( 0.05 )k.Disabled = falseend)end
        w:SetStateEnabled(Enum.HumanoidStateType.Jumping , true )w:SetStateEnabled(Enum.HumanoidStateType.Freefall , true )w:SetStateEnabled(Enum.HumanoidStateType.Running , true )w:SetStateEnabled(Enum.HumanoidStateType.Climbing , true )w.JumpPower =math.max ( 50 ,w.JumpPower )w.JumpHeight =math.max ( 7.2 ,w.JumpHeight )w:ChangeState(Enum.HumanoidStateType.Running )end
    )h.swapped = true
    if h.godmode then
        b4( true )end
    z4(e)
...
t4=function(...)
    if h.plot and(h.plot.Parent and(h.pen and(h.origin and h.plotVerified )))then
        return h.plot ,h.pen ,h.originend
    local e=r:FindFirstChild( "Plots" )
    if not e then
        return nil,nil,nilend
    local y=o.UserId
    local u=o.Name
        ...
        if r and(type(w)== "table" and type(w.OwnersBySlot )== "table" )then
            for r,w in pairs(w.OwnersBySlot )do
                if w==y or tostring(w)==tostring(y)or w==u then
                    ...
                    if j then
                        k= true
                        breakendendendendend
    if not j and c then
        local r,u=pcall(function(...)
            return c:InvokeServer()end)
    ...
    if not j then
        ...
        if y and y.Position.X <=(E+ 30 )then
            ...
            if r and u< 160 then
                j=rendendend
    if not j then
        j=e:FindFirstChild( "2" )or e:FindFirstChild( "1" )or(e:GetChildren())[ 1 ]end
    if not j then
        return nil,nil,nilend
    ...
    if not h.origin then
        h.origin =j:FindFirstChild( "CenterPoint" )or h.pen or j.PrimaryPartend
    if not h.pen then
        h.pen =h.originend
    return h.plot ,h.pen ,h.originend
s4=function(...)
    local e,r,y=t4()
    if r then
        return r.Position +Vector3.new ( 0 , 3.5 , 0 )end
    if y then
        return y.Position +Vector3.new ( 0 , 3.5 , 0 )end
    return Vector3.new ( 464.7 , 71.7 , -304 )end
p4=function(e,...)
    local r,y,u=t4()
    if not y then
        return nilend
    ...
    for r= 1 , 60 , 1 do
        ...
        if H then
            return Vendend
    return y.CFrame CFrame.new (math.random ( -8 , 8 ),w.Y / 2 + 1 ,math.random ( -8 , 8 ))end
B4=function(...)
    local e,r,y=t4()
                ...
                if r.Placement and r.Placement.LocalCFrame then
                    ...
                    if r then
                        ...
                        local k,a=pcall(function(...)
                            ...
                            [ "LocalCFrame" ]=j})end)
                        if k and a then
                            u=u+ 1 w[#w+ 1 ]=r.Positionendendendendendend
    local j={}
    local k=o.Character
    if k then
        for e,r in ipairs(k:GetChildren())do
            if m(r)then
                table.insert (j,r)endendend
    ...
    for e,r in ipairs(j)do
        ...
        if k then
            ...
            local r,a=pcall(function(...)
                return K:InvokeServer({[ "Uid" ]=j,[ "LocalCFrame" ]=e})end)
            if r and a~= false then
                u=u+ 1 w[#w+ 1 ]=k.Position H(string.format ( "[PlaceEgg] Placed egg %s from inventory" ,tostring(j)))end
            task.wait ( 0.04 )endend
    return uend
J4=function(e,...)
        ...
        if r.Placement then
            ...
            if y==nil then
                ...
                if p and(p.Assets and p.Assets [r.AssetCategory ])then
                    local e=p.Assets [r.AssetCategory ]u=(e and(e.Egg and e.Egg.GrowthTime ))or 30end
                local w=u/math.max ( 0.01 ,r.GrowthSpeedMultiplier or 1 )y=(k-e)>=wend
            if y then
                table.insert (j,{[ "uid" ]=e;
                [ "category" ]=r.AssetCategory or "Egg" })endendend
    if#j== 0 then
        h.isHatching = false
        return 0end
            ...
            if e and y~= false then
                ...
                local e,y=pcall(function(...)
                    if Q:IsA( "RemoteFunction" )then
                        ...
                        return trueendend)
                if e and y~= false then
                    a=a+ 1 h.hatched =((h.hatched or 0 ))+ 1 H(string.format ( "[+] [AutoHatch] Hatched %s (UID: %s) -> Total Hatched: %d" ,r.category ,tostring(r.uid ),h.hatched ))endendend
        )task.wait ( 0.04 )end
    task.wait ( 0.95 )h.isHatching = false H(string.format ( "[AutoHatch] Finished! Hatched %d eggs." ,a))
    return aend
i4=function(...)
    local e={}
    local y=r:FindFirstChild( "DEBRIS" )
    if y then
        for r,y in ipairs(y:GetChildren())do
...
g4=function(e,r,u,...)
    ...
    if not j then
        return falseend
    if k then
        k.AutoRotate = falseend
    local a=s4()e=math.max ( 100 ,e or h.glideSpeed or 1000 )
    ...
    while h.alive and(h.isReturning and os.clock ()<s)do
        ...
        local J=Bmath.min (math.abs (a.X -e.X ),Ho)
        local K=e.X +J
        local c=math.sign (a.Y -e.Y )
        local v=cmath.min (math.abs (a.Y -e.Y ),(Ho) 0.5 )
        local i=e.Y +v
        local R=s-e.Z
        local g=math.sign (R)math.min (math.abs (R),Ho)
        local Q=e.Z +g
        local P=i4()
        local N= false
        if e.X >E then
        ...
        if N then
            ...
            h.statusText =string.format ( "Tweening Home (%.0f studs | Z: %.0f | Spd: %.0f)" ,w,Q,H)endend
...
v4=function(e,r,y,...)
    ...
    if a> 8 then
        h.statusText = "[Place] Tweening back to base plot..." g4(e or h.glideSpeed or 1000 ,r, true )end
    ...
    local H=y4()h.statusText =string.format ( "Placed & Hatched (Left: %d)! Hands Free." ,H)end
local uk= 5 K4=function(e,...)
    ...
    if k> 8 then
        h.statusText = "[AutoPlace] Tweening home to base plot..." g4(h.glideSpeed or 1000 ,r, true )end
    if u then
        V4(j, 20 )u.CFrame =CFrame.new (j)u.AssemblyLinearVelocity =Vector3.zero u.AssemblyAngularVelocity =Vector3.zero
        if w then
            w.AutoRotate = trueendend
    ...
    for e= 5 , 1 , -1 do
        if not h.alive then
            breakend
        h.statusText =string.format ( "[AutoPlace] At Base: Resuming in %ds..." ,e)task.wait ( 1 )end
    ...
    if h.alive and O4==r then
        ...
        elseif e== "WARP" then
            h.autoFarmLoop = true h.pureTweenFarm = falseend
        Y4=eendend
c4=function(e,...)
    ...
    if h.batchStealCount >=uk then
        h.batchStealCount = 0 task.spawn (function(...) K4(e)end)
        return trueend
    return falseend
local function wk(e,r,u,w,...)
    ...
    if not k then
        return falseend
    if a then
        a.AutoRotate = falseend
    r=math.max ( 60 ,r or h.glideSpeed or 350 )
    local V=e.Position V4(V, 14 )pcall(function(...) o:RequestStreamAroundAsync(V)end
    ...
    while h.alive and(h.glidingToTarget and os.clock ()<s)do
        ...
        local v=cmath.min (math.abs (V.X -e.X ),rB)
        local i=e.X +v
        local R=(o<= 25 )and 1.2 or 0.5
        local g=math.sign (V.Y -e.Y )
        local Q=gmath.min (math.abs (V.Y -e.Y ),(rB)R)
        local P=e.Y +Q
        local N=K-e.Z
        local U=math.sign (N)math.min (math.abs (N),rB)
        local l=e.Z +U
        local D= false
        if o> 25 then
            local e=i4()
        ...
        if D then
            ...
            h.statusText =string.format ( "Gliding -> Egg (%.0f studs | H: %.0f)" ,j,o)endend
    k.CFrame =eCFrame.new ( 0 , 0.4 , 0 )k.AssemblyLinearVelocity =Vector3.zero k.AssemblyAngularVelocity =Vector3.zero
    if a then
        a.AutoRotate = trueend
    h.glidingToTarget = false
    return trueend
    ...
    if j then
        ...
        if w<= 535 and a> 510 then
            ...
            if a> 5 then
                ...
                if not j then
                    return falseend
                task.wait ( 0.04 )endendend
    return wk(e,r,y,u)end
Q4=function(e,r,...)
    local u=o.Character
    local w=u and u:FindFirstChild( "HumanoidRootPart" )
    local j=u and u:FindFirstChildOfClass( "Humanoid" )
    ...
    while h.alive and(h.isReturning and os.clock ()<s)do
        ...
        elseif e.X <=E then
            s=Aend
        local B=math.sign (a.X -e.X )
        local J=Bmath.min (math.abs (a.X -e.X ),sV)
        local K=e.X +J
        local c=math.sign (a.Y -e.Y )
        local v=cmath.min (math.abs (a.Y -e.Y ),(sV) 0.5 )
        local i=e.Y +v
        local R=k-e.Z
        local g=math.sign (R)math.min (math.abs (R),sV)
        local Q=e.Z +g
        local P=i4()
        local N= false
        for e,r in ipairs(P)do
        ...
        if N then
            ...
            h.statusText =string.format ( "Tweening to Safe Line (%.0f studs | X: %.0f)" ,o,e.X )endend
        ...
        for y,w in ipairs(y:GetDescendants())do
            if w:IsA( "BasePart" )and(w.Size.X >= 1.2 and w.Size.Z >= 1.2 )then
                if w.Position.Y <r then
                    r=w.Position.Y e=wendendend
        if e then
            return eend
        if y.PrimaryPart then
            return y.PrimaryPartend
...
f4=function(e,...)
    ...
    if V> 4 then
        ...
        while h.alive and(((Vector2.new (u.Position.X ,u.Position.Z )-Vector2.new (a.X ,a.Z ))).Magnitude > 4 and(os.clock ()-j< 4 ))do
            ...
            local V=(o-k)
            local H=V.Unit math.min (V.Magnitude ,rj)
            ...
            if w then
                ...
                if w.Sit then
                    w.Sit = falseend
                w:ChangeState(Enum.HumanoidStateType.Running )endendend
    if e and O4~=e then
        return falseend
    local H=os.clock ()
    while h.alive and(math.abs (u.Position.Y -a.Y )> 2 and(os.clock ()-H< 1.5 ))do
        ...
        local V=w+o u.CFrame =CFrame.new (V)u.AssemblyLinearVelocity =Vector3.zero u.AssemblyAngularVelocity =Vector3.zeroend
    ...
    if s<= 6 then
        pcall(function(...)
            if typeof(firetouchinterest)== "function" then
                firetouchinterest(u,k, 0 )task.wait ( 0.02 )firetouchinterest(u,k, 1 )endend
        )pcall(function(...)
            for r,y in ipairs(k:GetDescendants())do
                if y:IsA( "ProximityPrompt" )and y.Enabled then
                    if typeof(fireproximityprompt)== "function" then
                        fireproximityprompt(y)endendend
            if k.Parent then
                for r,y in ipairs(k.Parent :GetDescendants())do
                    if y:IsA( "ProximityPrompt" )and y.Enabled then
                        if typeof(fireproximityprompt)== "function" then
                            fireproximityprompt(y)endendendendend)
        if l then
            pcall(function(...) l:InvokeServer()end)end
            ...
            if y:IsA( "TextLabel" )and y.Visible then
                ...
                if e:find( "not enough money" )or e:find( "not enough cash" )or(e:find( "not enough" )and((e:find( "money" )or e:find( "cash" )or e:find( "coin" )or e:find( "fund" ))))then
                    ...
                    if e and(((e:IsA( "Frame" )or e:IsA( "CanvasGroup" )))and#e:GetChildren()<= 3 )then
                        e.Visible = falseendendendendend)end
local function ak(...)
    local e=o:FindFirstChild( "PlayerGui" )
    if not e then
        returnend
    ...
    pcall(function(...)
        for e,y in ipairs(e:GetDescendants())do
            task.spawn (r,y)end
        e.DescendantAdded :Connect(r)end
    )task.spawn (function(...)
        while h and h.alive do
            if h.hideNotEnoughMoney then
                kk()end
            task.wait ( 0.25 )endend)end
task.spawn (ak)
local function ok(e,...)
    if not e then
        return 0end
...
local function pk(...)
    ...
    tk=os.clock ()
    if D then
        pcall(function(...) D:InvokeServer()end)end
    local w=o.Character
    local j=w and w:FindFirstChild( "HumanoidRootPart" )pcall(function(...)
        for r,y in ipairs(r:GetDescendants())do
            if y:IsA( "ProximityPrompt" )and y.Enabled then
                if typeof(fireproximityprompt)== "function" then
                    fireproximityprompt(y, 0 )fireproximityprompt(y)endend
            if y:IsA( "GuiButton" )and y.Visible then
                ...
                if not string.find (u, "robux" )and(not string.find (u, "r%$" )and((string.find (u, "%$" )or string.find (u, "upgrade" )or string.find (u, "cash" )or(y.BackgroundColor3 and y.BackgroundColor3.G >y.BackgroundColor3.R ))))then
                    ...
                    elseif typeof(firesignal)== "function" and y.MouseButton1Click then
                        firesignal(y.MouseButton1Click )endendend
            if y:IsA( "BasePart" )and(y.Name :find( "Pad" )and j)then
                if((j.Position -y.Position )).Magnitude < 10 then
                    if typeof(firetouchinterest)== "function" then
                        firetouchinterest(j,y, 0 )task.wait ( 0.02 )firetouchinterest(j,y, 1 )endendendendend)end
                    ...
                    for u,w in ipairs(u:GetDescendants())do
                        if w:IsA( "GuiButton" )or w:IsA( "TextButton" )then
                            ...
                            if u:find( "unequip" )or(u:find( "equip" )and not u:find( "unequip" ))then
                                ...
                                local u=y:gsub( "Trail" , "" )e[u]= true e[u:lower()]= trueendendendendendend)end
    return eend
local function ck(e,...)
    if not e then
        return falseend
    pcall(function(...)
        ...
        elseif typeof(firesignal)== "function" and e.MouseButton1Click then
            firesignal(e.MouseButton1Click )endend)
    return trueend
local function vk(...)
    local e=Jk()
    local r=Kk()
        ...
        for r=#e, 1 , -1 do
            ...
            if u then
                ...
                if r then
                    ck(r)
                    if q then
                        pcall(function(...) q:InvokeServer(y.id )end)end
                    task.wait ( 0.2 )
                    return trueendendendend
    if q then
        for y=#e, 1 , -1 do
            ...
            if w then
                pcall(function(...) q:InvokeServer(u.id )end)
                return trueendendend
    return falseend
local ik= 0
local Rk= 8
                        ...
                        if y:IsA( "GuiButton" )or y:IsA( "TextButton" )then
                            ...
                            if not r:find( "robux" )and(not r:find( "r%$" )and(not r:find( "unequip" )and not r:find( "equip" )))then
                                if r:find( "%$" )or r:find( "buy" )then
                                    ck(y)e= true
                                    breakendendendendendend
            if C then
                pcall(function(...) C:InvokeServer(w.id )end
                )e= trueend
            if e then
                task.wait ( 0.3 )vk()
                breakendendendend
    ...
    if#u== 0 and(j and#j> 0 )then
        for e,r in ipairs(j)do
            ...
            if w and(k and not o)then
                ...
                [ "Area" ]=r.AreaId or "Field" })endendend
    if#u== 0 then
        return nilend
    table.sort (u,function(e,r,...)
        return e.Distance <r.Distanceend)
    local k=u[ 1 ]
    if k and w then
        for e,r in ipairs(w:GetChildren())do
...
local Nk= 350
local function Uk(...)
    local e=r:FindFirstChild( "OBJECTS" )or r:FindFirstChild( "Objects" )
    ...
    if u then
        local e=u:FindFirstChild( "Light Dark" )or u:FindFirstChild( "LightDark" )or u:FindFirstChild( "LightDark" )or u:FindFirstChild( "Light-Dark" )
        if e then
            return eend
        for e,r in ipairs(u:GetChildren())do
            ...
            if string.find (y, "light" )and string.find (y, "dark" )then
                return rendendend
    if y then
        local e=y:FindFirstChild( "Light Dark" )or y:FindFirstChild( "LightDark" )or y:FindFirstChild( "LightDark" )
        if e then
            return eend
    ...
    for e,r in ipairs(r:GetChildren())do
        local y=r.Name
        if y== "_OBJECTS" or y== "Objects" or y== "Areas" or y== "Map" then
            for e,r in ipairs(r:GetDescendants())do
                local y=string.lower (r.Name )
            ...
            for r,u in ipairs(r:GetChildren())do
                if u:IsA( "BasePart" )then
                    ...
                    if not e then
                        ...
                        e=Vector3.new (math.min (e.X ,k.X ),math.min (e.Y ,k.Y ),math.min (e.Z ,k.Z ))y=Vector3.new (math.max (y.X ,a.X ),math.max (y.Y ,a.Y ),math.max (y.Z ,a.Z ))endendend
            if e and y then
                u=CFrame.new (((e+y))/ 2 )w=y-eendend
        if u and w then
...
N4=function(...)
    ...
    local function j(e,...) e=tonumber(e)or 0
        if e>= 1000000000000 then
            return string.format ( "%.1fT" ,e/ 1000000000000 )end
        if e>= 1000000000 then
            return string.format ( "%.1fB" ,e/ 1000000000 )end
        if e>= 1000000 then
            return string.format ( "%.1fM" ,e/ 1000000 )end
        if e>= 1000 then
            return string.format ( "%.1fK" ,e/ 1000 )end
        return string.format ( "%.0f" ,e)end
    local function k(e,r,y,u,...)
        if e and e.PhysicalModel then
            ...
            if not r or r== "Egg" or r== "" then
                r=u:GetAttribute( "Category" )or u:GetAttribute( "AssetCategory" )or u.Nameendend
    ...
    local function a(r,y,...)
                ...
                if g then
                    ...
                    if r.Mutations and type(r.Mutations )== "table" then
                        for e,r in pairs(r.Mutations )do
                            local y=(type(r)== "table" and tonumber(r.Multiplier or r.Value ))or tonumber(r)or 1.5 a=ayend
                    ...
                    [ "Position" ]=r.BoundsCFrame.Position ,[ "Distance" ]=H,[ "Model" ]=r.PhysicalModel })endendend
        if#u== 0 then
            return nilend
        local function a(e,...)
        ...
        table.sort (u,function(e,r,...)
            ...
            if e.ZoneWeight ~=r.ZoneWeight then
                return e.ZoneWeight >r.ZoneWeightend
            if math.abs (e.RealIncome -r.RealIncome )> 1 then
                return e.RealIncome >r.RealIncomeend
            if math.abs (e.Scale -r.Scale )> 0.05 then
                return e.Scale >r.Scaleend
            return e.Distance <r.Distanceend)
        local o=u[ 1 ]
        local V={}
        for e= 1 ,math.min ( 3 ,#u), 1 do
            local r=u[e]table.insert (V,string.format ( "#%d %s[%s|%s] Score:%d $%s/s (%.1fx) dist=%dm" ,e,tostring(r.Category ),tostring(r.Rarity ),tostring(r.Area ),a(r),j(r.RealIncome ),tonumber(r.Scale )or 1 ,math.floor (tonumber(r.Distance )or 0 )))end
        if#V> 0 then
            H( "[AutoSteal v1.0] " ..table.concat (V, " | " ))end
        return oend
    local V=a( false , false )
    if not V then
        X4={}V=a( false , true )end
    if not V then
        e=h4( true )V=a( false , true )end
...
U4=function(e,u,w,j,...)
    ...
    local s=u.Position V4(s, 14 )h.currentTargetModel =w h.targetPosition =s a.AssemblyLinearVelocity =Vector3.zero a.AssemblyAngularVelocity =Vector3.zero Z4(k)pcall(function(...) o:RequestStreamAroundAsync(s)end)
    ...
    while not w4()and(os.clock ()<p and(h.alive and h.securingEgg ))do
        ...
        if e and(os.clock ()-B> 0.4 )then
            ...
            if not r and y== "CarriedByOther" then
                ...
                breakendend
        k:PivotTo(uCFrame.new ( 0 , 0.4 , 0 ))d4(w,s)
        if e and i then
            task.spawn (function(...) pcall(function(...)
                    if i:IsA( "RemoteFunction" )then
                        i:InvokeServer({[ "Uid" ]=e})i:InvokeServer(e)
                    else
                        i:FireServer({[ "Uid" ]=e})i:FireServer(e)endend)end)end
        y.Heartbeat :Wait()end
    if not w4()then
    ...
    while w4()and(os.clock ()<K and(h.alive and h.securingEgg ))do
        ...
        if not h.pureTweenFarm and(not h.autoFarmLoop and not h.teleporting )then
            breakend
        k:PivotTo(uCFrame.new ( 0 , 0.4 , 0 ))V4(s, 14 )
        if P and not c then
            task.spawn (function(...) pcall(function(...)
                    if P:IsA( "RemoteFunction" )then
                        P:InvokeServer()
                    else
                        P:FireServer()endend)end
            )c= trueend
        y.Heartbeat :Wait()end
...
l4=function(e,u,...)
    ...
    if not s then
        ...
        local w=e.CFrame CFrame.new ( 0 , 0.4 , 0 )pcall(function(...) o:RequestStreamAroundAsync(e.Position )end
        )V4(e.Position , 8 )
        if r> 60 then
            ...
            h.statusText = "[2/7] Aligning with Lake Egg..." j.CFrame =w j.AssemblyLinearVelocity =Vector3.zero task.wait ( 0.04 )end
            ...
            if e.Uid and i then
                task.spawn (function(...) pcall(function(...)
                        if i:IsA( "RemoteFunction" )then
                            ...
                            i:FireServer({[ "Uid" ]=e.Uid })endend)end)end
            y.Heartbeat :Wait()end
        s=select( 2 ,e4())
        if not w4()then
            t( "[-] Lake egg pickup failed" )h.statusText = "[-] Lake pickup failed" D4()
            return falseendend
    h.statusText = "[3/7] Pre-streaming Target..." pcall(function(...) o:RequestStreamAroundAsync(H)end
    ...
    if V then
        ...
        if not e and r== "CarriedByOther" then
            ...
            return falseendend
    h.currentTargetModel =e and e.Model h.targetPosition =H h.statusText = "[5/7] Warping to Target Egg..." V4(H, 8 )w:PivotTo(aCFrame.new ( 0 , 0.4 , 0 ))j.Anchored = true
    for e,r in ipairs(w:GetDescendants())do
        if r:IsA( "BasePart" )then
            r.AssemblyLinearVelocity =Vector3.zero r.AssemblyAngularVelocity =Vector3.zeroendend
        ...
        if O4==r then
            ...
            elseif e== "WARP" then
                ...
                if not h.isBatchPlacing then
                    h.batchStealCount = 0end
                H( "[FarmController] All farms DEACTIVATED. Bot idle." )endendend)end
                    ...
                    if not u then
                        ...
                        if w and(h.pureTweenFarm and(Y4== "TWEEN" and O4==u))then
                            if h.onTreadmill or L4()then
                                h.statusText = "[AutoSteal] Target found! Getting off treadmill..." M4()task.wait ( 0.08 )end
                            local j,k=k4(w.Uid )
                            ...
                            if not h.godmode then
                                b4( true )end
                            Z4(r)pcall(function(...) o:RequestStreamAroundAsync(w.Position )end)
                            ...
                            if h.pureTweenFarm and(Y4== "TWEEN" and((y.Position -w.Position )).Magnitude <= 22 )then
                                ...
                                if r then
                                    ...
                                    if h.autoGlide then
                                        ...
                                        h.statusText = "[AutoSteal] Secured! (Auto Return is OFF)" H( "[AutoSteal] Egg secured! Staying at target (Auto Return is OFF)." )end
                                    ...
                                    if O4==u and(h.pureTweenFarm and Y4== "TWEEN" )then
                                        t( "[AutoSteal] Guard Strike or Re-grab failed. Retrying with next egg..." )X4[w.Uid ]=os.clock ()+ 5 D4()endend
                            else
                                h.currentTargetModel =nil h.targetPosition =nil h.glidingToTarget = falseend
                        else
                            ...
                            if h.autoTreadmill and(not h.isBatchPlacing and not h.isHatching )then
                                if not h.onTreadmill and not L4()then
                                    ...
                                    h.statusText = "[AutoTreadmill] Running on treadmill (Waiting for eggs...)"end
                            else
                                h.statusText = "[AutoSteal] Scanning for targets..."endendendendendend)
        if not r then
            t( "[AutoSteal Loop Recovered]:" ,tostring(y))pcall(D4)end
        task.wait ( 1 )endend)
local qk=os.clock ()task.spawn (function(...)
                ...
                if y and u then
                    ...
                    if not r then
                        ...
                        if y and(h.autoFarmLoop and(Y4== "WARP" and O4==r))then
                            if h.onTreadmill or L4()then
                                h.statusText = "[SnipeLoop] Target found! Getting off treadmill..." M4()task.wait ( 0.08 )end
                            ...
                            if w then
                                ...
                                if h.autoGlide then
                                    ...
                                    h.statusText = "[SnipeLoop] Target secured! (Auto Return is OFF)" H( "[SnipeLoop] Snipe successful! Staying at target (Auto Return is OFF)." )end
                            ...
                            if h.autoTreadmill and(not h.isBatchPlacing and not h.isHatching )then
                                ...
                                h.statusText = "[SnipeLoop] Searching for targets..."endendendendendend)
        if not r then
            t( "[SnipeLoop Loop Recovered]:" ,tostring(y))pcall(D4)end
        task.wait ( 1 )endend
)task.spawn (function(...)
    while h.alive do
        local r,y=pcall(function(...)
            if h.autoTreadmill and(not h.pureTweenFarm and(not h.autoFarmLoop and(not h.isBatchPlacing and(not h.isHatching and(not h.teleporting and(not h.glidingToTarget and(not h.securingEgg and(not h.delivering and not h.isReturning ))))))))then
                ...
                if y and not w4()then
                    if not h.onTreadmill and not L4()then
                        h.statusText = "[AutoTreadmill] Idle without farm. Mounting treadmill..." f4()endendendend
        )task.wait ( 0.5 )endend
)task.spawn (function(...)
    while h.alive do
        pcall(function(...)
    ...
    while h.alive do
        if h.autoHatch and(not h.securingEgg and(not h.teleporting and not h.isHatching ))then
            pcall(function(...) J4( false )end)end
        task.wait ( 4 )endend)
local nk=nil
local function fk(e,...) pcall(function(...)
        if e:IsA( "BasePart" )then
            ...
            if e:IsA( "MeshPart" )then
                e.TextureID = "" pcall(function(...) e.RenderFidelity =Enum.RenderFidelity.Performanceend
                )pcall(function(...) e.CollisionFidelity =Enum.CollisionFidelity.Boxend)end
        ...
        elseif e:IsA( "Light" )or e:IsA( "PointLight" )or e:IsA( "SpotLight" )or e:IsA( "SurfaceLight" )then
            e.Enabled = false
        elseif e:IsA( "Highlight" )and e.Name ~= "EggESPHighlight" then
            e.Enabled = falseendend)end
local function Mk(...) h.performanceMode = true pcall(function(...)
        local e=r:FindFirstChild( "DiceHubEggESP" )
        if e then
            e:Destroy()end
        ...
        for e,r in ipairs(y:GetChildren())do
            if r:IsA( "PostEffect" )or r:IsA( "BloomEffect" )or r:IsA( "BlurEffect" )or r:IsA( "ColorCorrectionEffect" )or r:IsA( "SunRaysEffect" )or r:IsA( "DepthOfFieldEffect" )or r:IsA( "Atmosphere" )then
                pcall(function(...) r.Enabled = falseend)
            elseif r:IsA( "Sky" )then
                pcall(function(...) r.Parent =nilend)endend
        local u=workspace:FindFirstChildOfClass( "Terrain" )
        if u then
            pcall(function(...) u.Decoration = false u.WaterWaveSize = 0 u.WaterWaveSpeed = 0 u.WaterReflectance = 0 u.WaterTransparency = 0end)end
        ...
        pcall(function(...)
            if settings and(settings()).Rendering then
                (settings()).Rendering.QualityLevel = 1endend)end)end
local function Ik(...) h.performanceMode = false
    ...
    pcall(function(...)
        ...
        for e,r in ipairs(e:GetChildren())do
            if r:IsA( "PostEffect" )or r:IsA( "BloomEffect" )or r:IsA( "BlurEffect" )or r:IsA( "ColorCorrectionEffect" )or r:IsA( "SunRaysEffect" )or r:IsA( "DepthOfFieldEffect" )or r:IsA( "Atmosphere" )then
                pcall(function(...) r.Enabled = trueend)endend
        local r=workspace:FindFirstChildOfClass( "Terrain" )
        if r then
            pcall(function(...) r.Decoration = trueend)endend)end
local Lk= false
        ...
        if e then
            e:SendKeyEvent( true ,Enum.KeyCode.Escape , false ,game)task.wait ( 0.12 )e:SendKeyEvent( false ,Enum.KeyCode.Escape , false ,game)task.wait ( 0.35 )e:SendKeyEvent( true ,Enum.KeyCode.Escape , false ,game)task.wait ( 0.12 )e:SendKeyEvent( false ,Enum.KeyCode.Escape , false ,game)pcall(function(...)
                if typeof(e.SendTouchEvent )== "function" then
                    e:SendTouchEvent( 99999 , 0 , 15 , 15 )task.wait ( 0.04 )e:SendTouchEvent( 99999 , 2 , 15 , 15 )endend)endend
    )pcall(function(...)
        if typeof(mousemoverel)== "function" then
            mousemoverel( 1 , 0 )task.wait ( 0.05 )mousemoverel( -1 , 0 )endend)end
local function bk(...)
    if Lk then
        returnend
    Lk= true task.spawn (function(...)
        while h and(h.alive and h.antiAFK )do
            ...
            while r< 600 and(h and(h.alive and(h.antiAFK and Lk)))do
                task.wait ( 5 )r=r+ 5end
            if not h.antiAFK or not Lk then
                breakend
            Ek()end
        Lk= falseend)end
local function Ak(...) Lk= falseend
y.Heartbeat :Connect(function(...)
    ...
    if y and not((h and h.onTreadmill ))then
        if y.PlatformStand then
            y.PlatformStand = false y:ChangeState(Enum.HumanoidStateType.Running )end
        if y.Sit and((h.pureTweenFarm or h.autoFarmLoop or h.isReturning or h.glidingToTarget ))then
            y.Sit = false y:ChangeState(Enum.HumanoidStateType.Running )endend
    local u=r.Position
    local w=w4()
    if u.Y < 45 then
    ...
    if h.alive and(h.autoGlide and(w and(not a4()and u.X >E)))then
        task.spawn (function(...) Q4(h.glideSpeed )u4()h.isReturning = false h.delivering = falseend)endend)
...
local function Zk(e,...)
    ...
    for r= 1 ,#e, 4 do
        ...
        if a then
            ...
            if o then
                table.insert (u,string.char (bit32.bor (bit32.lshift (bit32.band (a, 3 ), 6 ),o)))endendend
    return table.concat (u)end
local zk= "DiceHubIcon.png"
        ...
        if u and u:IsA( "GuiObject" )then
            ...
            for e,r in ipairs(u:GetDescendants())do
                if r:IsA( "ImageLabel" )and((string.find (tostring(r.Image ), "117817408534198" )or string.find (r.Name :lower(), "outline" )))then
                    r.Visible = false r.ImageTransparency = 1endend
            local a=y
            if not a then
                local e,y,u=r:ToHSV()a=Color3.fromHSV (e,math.clamp (y 0.4 , 0.18 , 0.45 ), 0.18 )end
            u.ThemeTag =nil u.ImageColor3 =a u.ImageTransparency = 0.08endend)end
...
local function jM(...)
    local e=Instance.new ( "ScreenGui" )e.Name = "DiceLOADERSCREEN" e.ResetOnSpawn = false e.DisplayOrder = 9999999 e.ZIndexBehavior =Enum.ZIndexBehavior.Sibling e.AutoLocalize = false pcall(function(...)
        if syn and syn.protectgui then
            syn.protectgui (e)e.Parent =game:GetService( "CoreGui" )
        else
            e.Parent =o:FindFirstChild( "PlayerGui" )or game:GetService( "CoreGui" )endend)
    if not e.Parent then
        e.Parent =game:GetService( "CoreGui" )end
    ...
    local w=Instance.new ( "TextLabel" )w.Size =UDim2.new ( 1 , -28 , 0 , 24 )w.Position =UDim2.new ( 0 , 14 , 0 , 14 )w.BackgroundTransparency = 1 w.Text = "XenonByte Hub" w.TextColor3 =Color3.fromRGB ( 245 , 248 , 255 )w.TextSize = 18 w.Font =Enum.Font.GothamBold w.TextXAlignment =Enum.TextXAlignment.Left w.AutoLocalize = false w.Parent =r
    local j=Instance.new ( "TextLabel" )j.Size =UDim2.new ( 1 , -28 , 0 , 16 )j.Position =UDim2.new ( 0 , 14 , 0 , 38 )j.BackgroundTransparency = 1 j.Text = "Steal an Egg Suite v1.0" j.TextColor3 =Color3.fromRGB ( 140 , 150 , 175 )j.TextSize = 12 j.Font =Enum.Font.Gotham j.TextXAlignment =Enum.TextXAlignment.Left j.AutoLocalize = false j.Parent =r
    ...
    local V=Instance.new ( "Frame" )V.Size =UDim2.new ( 0 , 0 , 1 , 0 )V.BackgroundColor3 =Color3.fromRGB ( 0 , 0 , 0 )V.BorderSizePixel = 0 V.Parent =a;
    (Instance.new ( "UICorner" ,V)).CornerRadius =UDim.new ( 0 , 5 )
    local H=Instance.new ( "UIGradient" ,V)H.Color =ColorSequence.new ({ColorSequenceKeypoint.new ( 0 ,Color3.fromRGB ( 0 , 0 , 0 )),ColorSequenceKeypoint.new ( 1 ,Color3.fromRGB ( 255 , 255 , 255 ))})
    local t=Instance.new ( "TextLabel" )t.Size =UDim2.new ( 1 , -28 , 0 , 16 )t.Position =UDim2.new ( 0 , 14 , 0 , 94 )t.BackgroundTransparency = 1 t.Text = "Initializing XenonByte Hub..." t.TextColor3 =Color3.fromRGB ( 130 , 140 , 165 )t.TextSize = 11 t.Font =Enum.Font.Gotham t.TextXAlignment =Enum.TextXAlignment.Left t.AutoLocalize = false t.Parent =r task.spawn (function(...)
        for y= 1 , 100 , 1 do
            if not e.Parent then
                breakend
    ...
    local function s(o,...) task.spawn (function(...) task.wait ( 0.9 )
            ...
            (u:Create(t,H,{[ "TextTransparency" ]= 1 })):Play()task.wait ( 0.4 )pcall(function(...) e:Destroy()end)
            if o then
                o()endend)end
    return send
local kM={}kM.Gui =Instance.new ( "ScreenGui" )kM.Gui.Name = "DiceRESTOREBAR" kM.Gui.ResetOnSpawn = false kM.Gui.DisplayOrder = 999999 kM.Gui.ZIndexBehavior =Enum.ZIndexBehavior.Sibling kM.Gui.AutoLocalize = false pcall(function(...)
    if syn and syn.protectgui then
        syn.protectgui (kM.Gui )kM.Gui.Parent =game:GetService( "CoreGui" )
    else
        kM.Gui.Parent =o:FindFirstChild( "PlayerGui" )or game:GetService( "CoreGui" )endend)
if not kM.Gui.Parent then
    kM.Gui.Parent =game:GetService( "CoreGui" )end
kM.Btn =Instance.new ( "ImageButton" )kM.Btn.Name = "DiceSquareLogoButton" kM.Btn.Size =UDim2.fromOffset ( 46 , 46 )kM.Btn.Position =UDim2.new ( 0 , 20 , 0 , 20 )kM.Btn.BackgroundColor3 =Color3.fromRGB ( 18 , 18 , 24 )kM.Btn.Active = true kM.Btn.Selectable = true kM.Btn.Visible = false kM.Btn.ZIndex = 999999 kM.Btn.AutoLocalize = false kM.Btn.Parent =kM.Gui ;
...
(Instance.new ( "UICorner" ,kM.Logo )).CornerRadius =UDim.new ( 0 , 8 )kM.isDragging = false kM.dragStart =nil kM.startPos =nil kM.Btn.InputBegan :Connect(function(e,...)
    if e.UserInputType ==Enum.UserInputType.MouseButton1 or e.UserInputType ==Enum.UserInputType.Touch then
        kM.isDragging = true kM.dragStart =e.Position kM.startPos =kM.Btn.Positionendend
)w.InputEnded :Connect(function(e,...)
    if e.UserInputType ==Enum.UserInputType.MouseButton1 or e.UserInputType ==Enum.UserInputType.Touch then
        kM.isDragging = falseendend
    ...
    if kM.isDragging and((e.UserInputType ==Enum.UserInputType.MouseMovement or e.UserInputType ==Enum.UserInputType.Touch ))then
        local y=e.Position -kM.dragStart kM.Btn.Position =UDim2.new (kM.startPos.X .Scale ,kM.startPos.X .Offset +y.X ,kM.startPos.Y .Scale ,kM.startPos.Y .Offset +y.Y )endend)
local function aM(...) h.alive = false pcall(Ik)pcall(Ak)pcall(function(...) y:Set3dRenderingEnabled( true )end
    )pcall(function(...)
        local y=r:FindFirstChild( "DiceHubEggESP" )
        if y then
            y:Destroy()endend
    )pcall(D4)pcall(u4)
    if kM and kM.Gui then
        pcall(function(...) kM.Gui :Destroy()end)end
    if h.gui then
        pcall(function(...) h.gui :Destroy()end)end
    pcall(function(...)
        for r,y in ipairs(game.CoreGui :GetChildren())do
            if y.Name :find( "Dice" )or y.Name :find( "DesyncSniperUI" )or y.Name :find( "WindUI" )then
                y:Destroy()endendend)end
local function oM(...)
    local e=jM()
    local r=nil pcall(function(...) r=(loadstring(game:HttpGet( "https://raw.githubusercontent.com/Footagesus/WindUI/main/dist/main.lua" )))()end)
    local function j(e,...)
        if not e then
            returnend
        local y= false
        if r and r.Notify then
            local u=pcall(function(...) r:Notify(e)y= trueend)end
        if not y then
            pcall(function(...)
                (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(e.Title or "XenonByte Hub" ),[ "Text" ]=tostring(e.Content or "" );
                [ "Duration" ]= 3 })end)endend
    if r then
        pcall(function(...)
            local e=r.Notify
            if e then
                r.Notify =function(r,y,...)
                    local u=pcall(function(...) e(r,y)end)
                    if not u then
                        pcall(function(...)
                            (game:GetService( "StarterGui" )):SetCore( "SendNotification" ,{[ "Title" ]=tostring(y and y.Title or "XenonByte Hub" );
                            [ "Text" ]=tostring(y and y.Content or "" ),[ "Duration" ]= 3 })end)endendendend)
        ...
        local p=r:CreateWindow({[ "Title" ]= "XenonByte Hub" ;
        [ "Author" ]= "Steal An Egg V1" ;
        [ "Folder" ]= "DiceStealAnEgg" ;
        [ "Icon" ]=dk;
        ...
        p.IgnoreAlerts = true pcall(function(...)
            if p.UIElements and p.UIElements.Main then
                p.UIElements.Main .Visible = falseendend)
        ...
        local v=t task.spawn (function(...) task.wait ( 0.1 )
            ...
            if e then
                if e.AnchorPoint.Y ~= 0 then
                    local y=e.Size.Y .Offset > 0 and e.Size.Y .Offset or t e.Position =UDim2.new (e.Position.X .Scale ,e.Position.X .Offset ,e.Position.Y .Scale ,e.Position.Y .Offset -(ye.AnchorPoint.Y ))e.AnchorPoint =Vector2.new ( 0.5 , 0 )end
                e.ClipsDescendants = false mk(e)endend)
        local function i(...)
            local e=p.UIElements and p.UIElements.Main
            if not e or c then
                returnend
            ...
            if e.AnchorPoint.Y ~= 0 then
                local r=e.Size.Y .Offset > 0 and e.Size.Y .Offset or v e.Position =UDim2.new (e.Position.X .Scale ,e.Position.X .Offset ,e.Position.Y .Scale ,e.Position.Y .Offset -(r*e.AnchorPoint.Y ))e.AnchorPoint =Vector2.new ( 0.5 , 0 )end
            local k=e.Size.X .Scale
            local a=e.Size.X .Offset
            if K then
                if e.Size.Y .Offset >J then
                    v=e.Size.Y .Offsetend
                e.ClipsDescendants = true
                if w then
                    w.ClipsDescendants = trueend
                if j then
                    j.ClipsDescendants = trueend
                if r then
                    r.Visible = falseend
                if y then
                    y.Visible = falseend
                ...
                if j then
                    j.Visible = trueend
                local V=u:Create(e,TweenInfo.new ( 0.24 ,Enum.EasingStyle.Quart ,Enum.EasingDirection.Out ),{[ "Size" ]=UDim2.new (k,a, 0 ,J)})V:Play()task.delay ( 0.25 ,function(...) c= falseend)
            else
                e.Visible = true
                if j then
                    j.Visible = trueend
                ...
                if p.TabModule and p.TabModule.SelectedTab then
                    pcall(function(...) p.TabModule :SelectTab(p.TabModule.SelectedTab )end)end
                local H=u:Create(e,TweenInfo.new ( 0.24 ,Enum.EasingStyle.Quart ,Enum.EasingDirection.Out ),{[ "Size" ]=UDim2.new (k,a, 0 ,V)})H:Play()task.delay ( 0.25 ,function(...)
                    if not K then
                        ...
                        if w then
                            w.ClipsDescendants = falseend
                        if j then
                            j.ClipsDescendants = falseend
                        if r then
                            r.Visible = trueend
                        if y then
                            y.Visible = trueend
                        if p.TabModule and p.TabModule.SelectedTab then
                            pcall(function(...) p.TabModule :SelectTab(p.TabModule.SelectedTab )end)endend
                    c= falseend)endend
        p.Close =function(e,...) i()
            local r={}function r.Destroy(e,...) aM()end
            return rend
        local function R(...)
            if p.UIElements and p.UIElements.Main then
                ...
                if p.TabModule and p.TabModule.SelectedTab then
                    pcall(function(...) p.TabModule :SelectTab(p.TabModule.SelectedTab )end)endendend
        local function g(...)
            if p.UIElements and p.UIElements.Main then
                p.UIElements.Main .Visible = false kM.Btn.Visible = trueendend
        kM.Btn.MouseButton1Click :Connect(R)p.Destroy =function(e,...) g()end
        ...
        [ "Desc" ]=P.Farm.TeleportDesc ,[ "Icon" ]= "solar:magic-stick-3-bold" ,[ "Value" ]=h.autoFarmLoop ,[ "Callback" ]=function(e,...)
            ...
            if e then
                ...
                if U and U.Set then
                    N= true U:Set(e)N= falseendend)end
        W4=function(e,...) pcall(function(...)
                if l and l.Set then
                    N= true l:Set(e)N= falseendend)end
        ...
        [ "Callback" ]=function(...) task.spawn (function(...) j({[ "Title" ]= "Steal An Egg V1" ,[ "Content" ]=Gk[Xk].Notifications.PlaceStarted ,[ "Icon" ]= "loader" })h.statusText = "[Manual] Tweening to base..." v4(h.glideSpeed ,nil, true )j({[ "Title" ]= "Steal An Egg V1" ,[ "Content" ]=Gk[Xk].Notifications.PlaceDone ,[ "Icon" ]= "check-circle" })end)end
        ...
        [ "Callback" ]=function(e,...) h.autoPlaceEvery5 =e
            if not e then
                h.batchStealCount = 0end
            local r=Gk[Xk]or Gk.EN j({[ "Title" ]= "Auto Place (Every 5)" ,[ "Content" ]=e and((r.Notifications.AutoPlaceStarted or "Auto Place (Every 5) enabled" ))or(r.Notifications.AutoPlaceStopped or "Auto Place (Every 5) disabled" ),[ "Icon" ]=e and "check-circle" or "x-circle" })end
        })Fk.togAutoHatch =hk:Toggle({[ "Title" ]=P.Farm.HatchTitle ;
        [ "Desc" ]=P.Farm.HatchDesc ,[ "Icon" ]= "solar:star-bold" ;
        [ "Value" ]=h.autoHatch ,[ "Callback" ]=function(e,...) h.autoHatch =e j({[ "Title" ]= "Auto Hatch" ,[ "Content" ]=e and Gk[Xk].Notifications.HatchStarted or Gk[Xk].Notifications.HatchStopped ,[ "Icon" ]=e and "check-circle" or "x-circle" })end
        ...
        [ "Callback" ]=function(e,...) h.autoGlide =e j({[ "Title" ]= "Auto Return" ;
            ...
            [ "Icon" ]=e and "check-circle" or "x-circle" })end
        ...
        [ "Value" ]=h.autoUpgradeTreadmill ,[ "Callback" ]=function(e,...) h.autoUpgradeTreadmill =e x()
            local r=Gk[Xk]or Gk.EN j({[ "Title" ]=(Xk== "TH" )and "อัปเกรดลู่วิ่ง" or "Upgrade Treadmill" ,[ "Content" ]=e and((r.Notifications.UpgradeTreadmillStarted or "Auto Upgrade Treadmill enabled" ))or(r.Notifications.UpgradeTreadmillStopped or "Auto Upgrade Treadmill disabled" ),[ "Icon" ]=e and "check-circle" or "x-circle" })end
        ...
        [ "Callback" ]=function(e,...)
            ...
            local function y(e,...)
                if type(e)== "table" then
                    e=e.Title or e.Name or e[ 1 ]or ""end
        ...
        [ "Callback" ]=function(e,...)
            if e then
                ...
                disableDesyncGodmode()j({[ "Title" ]= "Godmode" ,[ "Content" ]=Gk[Xk].Notifications.GodmodeStopped ,[ "Icon" ]= "shield-off" })endend
        ...
        [ "Callback" ]=function(...) pcall(M4)pcall(C4)pcall(D4)j({[ "Title" ]= "Unstick" ;
            [ "Content" ]=Gk[Xk].Notifications.UnstickDone ,[ "Icon" ]= "check" })end
        ...
        [ "Max" ]= 1000 ;
        [ "Default" ]=h.glideSpeed or 1000 },[ "Callback" ]=function(e,...) h.glideSpeed =e Y(e)end
        })Fk.secDashboard =Tk:Section({[ "Title" ]=P.Settings.SecDashboard })Fk.paraLiveDash =Tk:Paragraph({[ "Title" ]=P.Settings.DashTitle ;
        [ "Desc" ]=string.format ( "Status: Ready\nFarm Mode: Idle\nCarried Eggs: 0\nFlight Speed: %d Studs/s" ,h.glideSpeed or 1000 )})Fk.secUI =Tk:Section({[ "Title" ]=P.Settings.SecUI })Fk.dropLang =Tk:Dropdown({[ "Title" ]=P.Settings.LangTitle ,[ "Values" ]={ "English" , "ไทย" },[ "Value" ]=(Xk== "EN" and "English" or "ไทย" ),[ "Callback" ]=function(e,...)
            local r=(e== "ไทย" )and "TH" or "EN"
            if r~=Xk then
                Xk=r wM(Xk)pcall(x)j({[ "Title" ]=(Xk== "TH" )and "ภาษา" or "Language" ,[ "Content" ]=Gk[Xk].Notifications.LangSwitched ,[ "Icon" ]= "check-circle" })endend
        })Fk.sliderTransp =Tk:Slider({[ "Title" ]=P.Settings.TranspTitle ;
        [ "Desc" ]=P.Settings.TranspDesc ;
        [ "Step" ]= 5 ;
        [ "Value" ]={[ "Min" ]= 0 ;
        [ "Max" ]= 90 ,[ "Default" ]= 0 },[ "Callback" ]=function(e,...) Q(e)end
        })Fk.dropTheme =Tk:Dropdown({[ "Title" ]=P.Settings.ThemeTitle ;
        [ "Values" ]={ "Dark" ;
        "Rose" , "Plant" ;
        "Red" , "Sky" , "Purple" },[ "Value" ]= "Dark" ;
        [ "Callback" ]=function(e,...) pcall(function(...) r:SetTheme(e)end)end
        ...
        [ "Desc" ]=(P.Settings and P.Settings.PerformanceDesc )or "Disables shadows, textures, particles, and shaders for maximum FPS smoothness" ,[ "Icon" ]= "solar:bolt-bold" ,[ "Value" ]=h.performanceMode ,[ "Callback" ]=function(e,...) h.performanceMode =e x()
            if e then
                ...
                Ik()end
            local r=Gk[Xk]or Gk.EN j({[ "Title" ]= "Performance Mode" ,[ "Content" ]=e and((r.Notifications.PerformanceStarted or "Performance Mode enabled" ))or(r.Notifications.PerformanceStopped or "Performance Mode disabled" ),[ "Icon" ]=e and "check-circle" or "x-circle" })end
        ...
        [ "Value" ]=h.disable3D ,[ "Callback" ]=function(e,...) h.disable3D =e x()pcall(function(...) y:Set3dRenderingEnabled(not e)end)
            local r=Gk[Xk]or Gk.EN j({[ "Title" ]= "3D Rendering" ,[ "Content" ]=e and((r.Notifications.Disable3DStarted or "3D Rendering disabled (GPU Saver)" ))or(r.Notifications.Disable3DStopped or "3D Rendering restored" ),[ "Icon" ]=e and "check-circle" or "x-circle" })end
        ...
        [ "Value" ]=h.antiAFK ,[ "Callback" ]=function(e,...) h.antiAFK =e x()
            if e then
                ...
                Ak()end
            local r=Gk[Xk]or Gk.EN j({[ "Title" ]= "Anti-AFK" ,[ "Content" ]=e and((r.Notifications.AntiAFKStarted or "Safe Anti-AFK enabled" ))or(r.Notifications.AntiAFKStopped or "Safe Anti-AFK disabled" ),[ "Icon" ]=e and "check-circle" or "x-circle" })end
        ...
        [ "Callback" ]=function(...) pcall(D4)pcall(u4)j({[ "Title" ]= "Reset State" ;
            [ "Content" ]= "Character state reset successfully" ,[ "Icon" ]= "check-circle" })end
        })Fk.btnRejoin =Tk:Button({[ "Title" ]=P.Settings.RejoinTitle ;
        [ "Desc" ]=P.Settings.RejoinDesc ;
        [ "Icon" ]= "solar:logout-2-bold" ,[ "Callback" ]=function(...) pcall(function(...) TeleportService:TeleportToPlaceInstance(game.PlaceId,game.JobId,o)end)end
        })Fk.btnUnload =Tk:Button({[ "Title" ]=P.Settings.UnloadTitle ,[ "Desc" ]=P.Settings.UnloadDesc ;
        [ "Icon" ]= "solar:trash-bin-trash-bold" ,[ "Callback" ]=function(...) aM()end
                ...
                pcall(function(...)
                    ...
                    if B then
                        ...
                        elseif h.delivering then
                            e=Color3.fromRGB ( 16 , 185 , 129 )end
                        pcall(function(...)
                            if B.SetTitle then
                                B:SetTitle(((y and "สถานะ: " or "Status: " ))..u)end
                            if B.SetColor then
                                B:SetColor(e)endend)end
                    if Fk.paraLiveDash and Fk.paraLiveDash.SetDesc then
                        ...
                        elseif Y4== "WARP" then
                            e=y and "ขโมยไข่ (วาร์ป)" or "Auto Steal (Teleport)"end
                        local j=y and "สถานะ: %s\nโหมดฟาร์ม: %s\nจำนวนไข่ในตัว: %d ฟอง\nความเร็วบิน: %d Studs/s" or "Status: %s\nFarm Mode: %s\nCarried Eggs: %d\nFlight Speed: %d Studs/s"
                        local k=string.format (j,u,e,r,h.glideSpeed or 1000 )pcall(function(...) Fk.paraLiveDash :SetDesc(k)end)endend
                )task.wait ( 0.5 )endend
        )e(R)
        returnend
    if h.gui then
    ...
    local k=Instance.new ( "ScreenGui" )k.Name = "DesyncSniperUIv415" k.ResetOnSpawn = false k.DisplayOrder = 99999 k.ZIndexBehavior =Enum.ZIndexBehavior.Sibling k.AutoLocalize = false
    ...
    local l=Instance.new ( "TextLabel" )l.Size =UDim2.new ( 1 , -90 , 0 , 22 )l.Position =UDim2.new ( 0 , 12 , 0 , 6 )l.BackgroundTransparency = 1 l.Text = "XenonByte Hub (Fallback)" l.TextColor3 =B l.TextSize = 14 l.Font =Enum.Font.GothamBold l.TextXAlignment =Enum.TextXAlignment.Left l.AutoLocalize = false l.Parent =U
    local D=Instance.new ( "TextLabel" )D.Size =UDim2.new ( 1 , -90 , 0 , 14 )D.Position =UDim2.new ( 0 , 12 , 0 , 26 )D.BackgroundTransparency = 1 D.Text = "Steal an Egg v1.0" D.TextColor3 =Color3.fromRGB ( 0 , 255 , 160 )D.TextSize = 11 D.Font =Enum.Font.Gotham D.TextXAlignment =Enum.TextXAlignment.Left D.AutoLocalize = false D.Parent =U
    ...
    local L=Instance.new ( "UIPadding" )L.PaddingTop =UDim.new ( 0 , 8 )L.PaddingBottom =UDim.new ( 0 , 12 )L.PaddingLeft =UDim.new ( 0 , 10 )L.PaddingRight =UDim.new ( 0 , 10 )L.Parent =n C.MouseButton1Click :Connect(function(...) g=not g C.Text =g and "+" or "-" ;
        (u:Create(Q,TweenInfo.new ( 0.25 ,Enum.EasingStyle.Quart ,Enum.EasingDirection.Out ),{[ "Size" ]=g and UDim2.new ( 0 , 330 , 0 ,R)or UDim2.new ( 0 , 330 , 0 ,i)})):Play()end
    )q.MouseButton1Click :Connect(function(...) aM()end)
    ...
    local function b(e,r,y,w,j,k,...)
        ...
        local function i(e,...) p=e
            ...
            [ "BackgroundColor3" ]=p and v or c})):Play()end
        H.MouseButton1Click :Connect(function(...)
            local e=not p i(e)k(e)end)
        return iend
    local function A(e,r,y,u,w,...)
        ...
        (Instance.new ( "UICorner" ,o)).CornerRadius =UDim.new ( 0 , 6 )o.MouseButton1Click :Connect(w)end
    ...
    A( "Single Steal (Teleport)" , "Teleport to steal 1 target egg and return" ,Color3.fromRGB ( 59 , 130 , 246 ), 13 ,function(...) task.spawn (function(...)
            ...
            if e then
                ...
                if y then
                    pcall(u4)
                    if h.autoGlide then
                        Q4(h.glideSpeed )u4()endendendend)end
    )E( "PLACE EGG" , 20 )A( "Place Egg" , "Tween home, place all carried eggs & hatch" ,Color3.fromRGB ( 16 , 215 , 130 ), 21 ,function(...) task.spawn (function(...) h.statusText = "[Manual] Depositing eggs..." g4(h.glideSpeed )v4()u4()h.isReturning = false h.delivering = falseend)end
        ...
        if not e then
            h.batchStealCount = 0endend
    )b( "Auto Hatch" , "Automatically hatch ready eggs continuously" ,h.autoHatch ,Color3.fromRGB ( 16 , 185 , 129 ), 22 ,function(e,...) h.autoHatch =eend
    )b( "Auto Return" , "Automatically return to safe area after stealing" ,h.autoGlide ,Color3.fromRGB ( 245 , 158 , 11 ), 23 ,function(e,...) h.autoGlide =eend
        ...
        if e then
            ...
            disableDesyncGodmode()endend
    )A( "Get Out Treadmill" , "Instantly escape from treadmill or gear" ,Color3.fromRGB ( 249 , 115 , 22 ), 32 ,function(...) pcall(M4)pcall(C4)pcall(D4)end
    ...
    local T=Instance.new ( "TextLabel" )T.Size =UDim2.new ( 0 , 70 , 0 , 24 )T.Position =UDim2.new ( 1 , -80 , 0.5 , -12 )T.BackgroundColor3 =V T.Text =string.format ( "%d Studs/s" ,h.glideSpeed or 1000 )T.TextColor3 =Color3.fromRGB ( 0 , 255 , 160 )T.TextSize = 11 T.Font =Enum.Font.GothamBold T.AutoLocalize = false T.Parent =F;
    ...
    (Instance.new ( "UICorner" ,m)).CornerRadius =UDim.new ( 0 , 6 )W.MouseButton1Click :Connect(function(...) h.glideSpeed =math.max ( 100 ,((h.glideSpeed or 1000 ))- 25 )T.Text =string.format ( "%d Studs/s" ,h.glideSpeed )Y(h.glideSpeed )end
    )m.MouseButton1Click :Connect(function(...) h.glideSpeed =math.min ( 1000 ,((h.glideSpeed or 1000 ))+ 25 )T.Text =string.format ( "%d Studs/s" ,h.glideSpeed )Y(h.glideSpeed )end
    )A( "Reset Character State" , "Clear velocity, cancel push & unfreeze" ,Color3.fromRGB ( 99 , 102 , 241 ), 42 ,function(...) pcall(D4)pcall(u4)end
    )A( "Unload Script" , "Destroy UI and stop all background loops" ,Color3.fromRGB ( 153 , 27 , 27 ), 43 ,function(...) aM()end
    ...
    for e,r in ipairs(M)do
        ...
        local function u(...)
            ...
            if e then
                ...
                y.BackgroundColor3 =Color3.fromRGB ( 28 , 32 , 44 )y.TextColor3 =Color3.fromRGB ( 140 , 150 , 170 )y.Text =rendend
        u()y.MouseButton1Click :Connect(function(...)
            if not h.selectedZones then
                h.selectedZones ={}end
            h.selectedZones [r]=not((h.selectedZones [r]== true ))u()x()w4.Text =string.format ( "📍 Target Zones (%d/12 Active)" ,r4())end
        )y.Parent =a4 V4[r]=yend
    local H4= false e4.MouseButton1Click :Connect(function(...) H4=not H4 a4.Visible =H4 a4.Size =H4 and UDim2.new ( 1 , 0 , 0 , 240 )or UDim2.new ( 1 , 0 , 0 , 0 )k4.Text =H4 and "▲" or "▼"end)
    local function t4(...)
        local e= 0
    ...
    for e,r in ipairs(X)do
        ...
        u()y.MouseButton1Click :Connect(function(...)
            if not h.selectedRarities then
                h.selectedRarities ={}end
            h.selectedRarities [r]=not((h.selectedRarities [r]== true ))u()x()p4.Text =string.format ( "🥚 Target Rarities (%d/%d Active)" ,t4(),#X)end
        )y.Parent =K4end
    local i4= false s4.MouseButton1Click :Connect(function(...) i4=not i4 K4.Visible =i4 K4.Size =i4 and UDim2.new ( 1 , 0 , 0 , 160 )or UDim2.new ( 1 , 0 , 0 , 0 )J4.Text =i4 and "▲" or "▼"end
    )e(function(...) Q.Visible = trueend)end
H( "[+] Initializing XenonByte Hub x WindUI v1.0 (Steal an Egg Edition)..." )oM()task.spawn (function(...) task.wait ( 0.5 )A4()b4( true )C4()
    if o.Character then
        z4(o.Character )end
    u4()H( "[+] Auto Humanoid Swap & Rigid Joint Locking Active." )end
)o.CharacterAdded :Connect(function(e,...) task.wait ( 0.6 )
    if h.alive then
        D4()n4()C4()A4()b4( true )z4(e)u4()endend)
if h.performanceMode then
    task.spawn (Mk)end
if h.disable3D then
    pcall(function(...) y:Set3dRenderingEnabled( false )end)end
if h.antiAFK then
    task.spawn (bk)end
H( "[+] XenonByte Hub v1.0 Ready! All-In-One Built-in Anti-AFK & Prometheus Ready!" )